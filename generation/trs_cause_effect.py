...
class GLSTM(nn.Module):
    def __init__(self, embedding, emb_dim, hidden_dim, hop_num):
        super(GLSTM, self).__init__()
        self.embedding = embedding
        self.device = config.device
        self.relation_embd = nn.Embedding(47*2, emb_dim)
        ...
        self.lstm = GCNLSTM(emb_dim, hidden_dim)
...
class GCNLSTM(nn.Module):
    def __init__(self, embed_size, hidden_dim):
        ...
        self.cause_lstm = nn.GRU(embed_size, hidden_dim, batch_first=True, bidirectional=True)
        self.cause_linear = nn.Linear(hidden_dim * 2, hidden_dim, bias=False)
...
class CauseEffect(nn.Module):
    def __init__(self, vocab, decoder_number, act_num=8, model_file_path=None, load_optim=False):
        ...
        self.glstm = GLSTM(self.embedding, config.emb_dim, config.hidden_dim, config.hop_num)
