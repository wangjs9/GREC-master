...
class Transformer_CauseHop(nn.Module):
    def __init__(self, vocab, hop_num, decoder_number, model_file_path=None, is_eval=False, load_optim=False):
        """
        vocab: a Lang type data, which is defined in data_reader.py
        decoder_number: the number of classes
        """
        super(Transformer_CauseHop, self).__init__()
        ...
        self.W_s = nn.ModuleList(
            [nn.Linear(config.emb_dim, config.emb_dim, bias=False) for _ in range(self.hop_num)])
        self.W_n = nn.ModuleList(
            [nn.Linear(config.emb_dim, config.emb_dim, bias=False) for _ in range(self.hop_number)])
        self.W_r = nn.ModuleList(
            [nn.Linear(config.emb_dim, config.emb_dim, bias=False) for _ in range(self.hop_number)])

        self.cause_encoder = Encoder(config.emb_dim, config.hidden_dim, num_layers=config.hop, num_heads=config.heads,
        ...
        if model_file_path is not None:
            ...
            self.cause_encoder.load_state_dict(state['caz_encoder_dict'])
    ...
    def save_model(self, running_avg_ppl, iter, f1_g, f1_b, ent_g, ent_b):
        ...
        state = {
            ...
            'caz_encoder_dict': self.cause_encoder.state_dict(),
    ...
    def multi_layer_gcn(self, concept_hidden, relation_hidden, head, tail, concept_label, triple_label,
                             layer_number=2):
        for i in range(layer_number):
            concept_hidden, relation_hidden = self.gcn(concept_hidden, relation_hidden, head, tail, concept_label,
                                                            triple_label, i)
    ...
    def gcn(self, concept_hidden, relation_hidden, head, tail, triple_label, layer_idx):
        ...
        o = concept_hidden.gather(1, head.unsqueeze(2).expand(batch_size, max_trp_size, hidden_size))
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        concept_ids, relation, head, tail, concept_label, triple_label = get_graph_from_batch(batch)
        ...
        cause_encoder_outputs = self.cause_encoder(self.embedding(cause_batch), mask_cause)

        ## graph processing
        memory = self.embedding(concept_ids)
        rel_repr = self.embedding(relation)
        node_repr, rel_repr = self.multi_layer_comp_gcn(memory, rel_repr, head, tail, concept_label, triple_label,
        ...
        ## integration
        encoder_outputs = torch.cat((encoder_outputs, triple_repr), dim=-1)
    ...
    def decoder_greedy(self, batch, max_dec_step=30):
        ...
        enc_batch_extend_vocab, _ = get_input_from_batch(batch)

        mask_src = enc_batch.data.eq(config.PAD_idx).unsqueeze(1)
        emb_mask = self.embedding(batch["mask_input"])
        cazpos = self.causeposembeding(enc_causepos)
        encoder_outputs = self.encoder(self.embedding(enc_batch) + emb_mask + cazpos,
                                       mask_src, enc_causeprob)

        ## caz_encoder
        mask_caz = cause_batch.data.eq(config.PAD_idx).unsqueeze(1)
        caz_encoder_outputs = self.cause_encoder(self.embedding(cause_batch), mask_caz)
    ...
    def decoder_topk(self, batch, max_dec_step=30):
        ...
        for i in range(max_dec_step + 1):
            ...
            logit = self.generator(out, attn_dist, enc_batch_extend_vocab, attn_dist_db=None)
